"""
This code is based on the Muon optimizer of PyTorch 2.11, with additional support for the Polar Express method and 8-bit quantization.
"""
...
def optimal_composition (l ,u , num_iters , cushion =0.02407327424182761):
    polarexpress_coefficients = []
    for _ in range ( num_iters ) :
        ...
        polarexpress_coefficients . append (( a , b , c ) )
        l = a * l + b * l **3 + c * l ** 5
        u = 2 - l
    return polarexpress_coefficients

@torch.compile
def _zeropower_via_polarexpress(grad, coeffs_list, eps=0.01):
    shape = grad.shape
    if len(shape) > 2:
        grad = grad.reshape(shape[0], -1)
    ...
    needs_transpose = X.size(0) > X.size(1)
    if needs_transpose:
        X = X.T

    X.div_(X.norm().clamp(min=eps))
    for i in range(len(coeffs_list)):
        a, b, c = coeffs_list[i][0], coeffs_list[i][1], coeffs_list[i][2]
        gram_matrix = X @ X.T
...
class Muon(Optimizer):
    def __init__(
        ...
        polar_express: bool = True,
        eps: float = EPS,
        ns_steps: int = DEFAULT_NS_STEPS,
        pe_l: float = 0.001,
        pe_u: float = 1.0,
        pe_steps: int = DEFAULT_PE_STEPS,
        ...
        self.all_coeffs = []
        polarexpress_coefficients = optimal_composition(l=pe_l, u=pe_u, num_iters=pe_steps, cushion =0.02407327424182761)
        defaults = {
            "lr": lr,
            ...
            "polarexpress_coefficients": polarexpress_coefficients,
            "quant": quant
        }
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            muon(
                ...
                polar_express=polar_express,
                polarexpress_coefficients=group["polarexpress_coefficients"],
                quant = group["quant"]
            )
        return loss
...
def _single_tensor_muon(
    ...
    *,
    ...
    has_complex: bool,
    polar_express: bool,
    polarexpress_coefficients: list[tuple[float, float, float]],
    quant: bool
) -> None:
    lr = _to_scalar(lr)
    if has_complex:
    ...
    for i, param in enumerate(params):
        if quant:
            ...
            update = (_zeropower_via_polarexpress(update, polarexpress_coefficients) if polar_express 
              else _zeropower_via_newtonschulz(update, ns_coefficients, ns_steps, eps))
    
            adjusted_lr = _adjust_lr(lr, adjust_lr_fn, param.shape)
    
            param.mul_(1 - lr * weight_decay)
            param.add_(update, alpha=-adjusted_lr)
            muon_momentum_bufs[i] = to_affine_quantized_intx(
                                                            buf,                                     
                                                            mapping_type=MappingType.SYMMETRIC,       
                                                            target_dtype=torch.int8,
                                                            block_size = buf.shape,
